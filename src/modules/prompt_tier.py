# modules_model_tier.py
# Cost-Performance Balance Strategy: Prompt Complexity Tiers
#
# This version uses the same LLM but different prompt complexities for cost tiers:
# - LOW (0.2): Simple direct prompts
# - MID (0.5): Chain of Thought (CoT) prompts
# - HIGH (1.0): Plan→Act→Reflect→Final multi-step reasoning prompts
#
# All use the same model, but different reasoning depth
...
class ModuleOutput:
    ...
    cost_level: int  # Cost level used
    metadata: Dict[str, Any] = field(default_factory=dict)  # Additional metadata


@dataclass
class MemoryModuleOutputs:
    """Store the processing output of each module for memory"""
    filter_output: Optional[ModuleOutput] = None        # Module 1 output
    entity_relation_output: Optional[ModuleOutput] = None  # Module 2 output
    temporal_relation_output: Optional[ModuleOutput] = None  # Module 3 output
    topic_relation_output: Optional[ModuleOutput] = None    # Module 5 output
    summary_output: Optional[ModuleOutput] = None       # Module 4 output
...
# ============================================================================
# Prompt Complexity Helper
# ============================================================================
# Note: All cost levels use the same model, just different prompt complexities

def _extract_tokens_from_llm_response(ret) -> Tuple[int, int]:
    """
    Extract token count from the return value of get_llm_response
    
    Args:
        ret: Return value of get_llm_response, format: [(q_id, response, (input_tokens, output_tokens), success), ...]
    
    Returns:
        (input_tokens, output_tokens), returns (0, 0) if failed
    """
    if len(ret) == 0:
        return 0, 0
...
def _calculate_token_cost(model_name: str, ret) -> float:
    """
    Calculate raw cost based on model name and LLM response (without normalization)

    Args:
        model_name: Model name
        ret: Return value of get_llm_response

    Returns:
        Raw cost (in USD), returns 0.0 if failed
    """
...
class Module1_Filter:
    ...
    Uses the same LLM with different prompt complexities:
    - Low cost: Simple direct prompt
    - Mid cost: Chain of Thought (CoT) prompt
    - High cost: Plan→Act→Reflect multi-step reasoning prompt
    """

    # Cost weights for prompt complexity tiers (deprecated, now using token-based cost)
    ...
    def execute(
        ...
        Returns:
            (Filtered memory list, cost value (token count))
        """
        if cost_level == CostLevel.NOOP:
            return memories[:self.top_k], 0.0
        
        model_name = getattr(self.args, 'model', 'meta/llama-3.3-70b-instruct')
        if cost_level == CostLevel.LOW:
            result, ret = self._low_cost_filter(query, memories)
        elif cost_level == CostLevel.MID:
            result, ret = self._mid_cost_filter(query, memories)
        else:  # HIGH
    ...
    def _low_cost_filter(self, query: str, memories: List[Any]) -> Tuple[List[Any], List]:
        """
        Low cost: Simple direct prompt
        """
        if len(memories) == 0:
        ...
        prompt = MODULE1_FILTER_PROMPT_Direct.format(query=query, memories_text=memories_text)
    ...
    def _mid_cost_filter(self, query: str, memories: List[Any]) -> Tuple[List[Any], List]:
        """
        Mid cost: Chain of Thought (CoT) prompt
        """
        if len(memories) == 0:
        ...
        # Build batch scoring prompt with CoT
        memories_text = ""
        for idx, mem in enumerate(memories):
            content_preview = mem.content
            mem_metadata = []
        ...
        prompt = MODULE1_FILTER_PROMPT_COT.format(query=query, memories_text=memories_text)
    ...
    def _high_cost_filter(self, query: str, memories: List[Any]) -> Tuple[List[Any], List]:
        """
        High cost: Plan→Act→Reflect multi-step reasoning prompt
        """
        if len(memories) == 0:
        ...
        # Build batch scoring prompt with Plan→Act→Reflect
        memories_text = ""
        for idx, mem in enumerate(memories):
            content_preview = mem.content
            mem_metadata = []
    ...
    def _parse_scores(self, ret, memories: List[Any]) -> List[Any]:
        ...
        # Parse scores - handle common markdown code block wrapping
        try:
            ...
            if "<answer>" in response and "</answer>" in response:
                ...
                # print("[Module1_Filter] response: ", response)
            # Handle cases wrapped in ```json ... ``` or ``` ... ```
            ...
            try:
                ...
                try:
                    ...
                    print(f"[Module1_Filter._parse_scores] JSON repair failed: {repair_error}, returning top_k memories")
                    return memories[:self.top_k]
            ...
            # Return top_k memories when parsing fails
...
class Module2_EntityRelation:
    ...
    def execute(
        ...
        Returns:
            (Entity relation list, cost value (based on token count))
        """
        if cost_level == CostLevel.NOOP:
            return [], 0.0
        ...
        model_name = getattr(self.args, 'model', 'meta/llama-3.3-70b-instruct')
        if cost_level == CostLevel.LOW:
            result, ret = self._low_cost_extract(query, memories)
        elif cost_level == CostLevel.MID:
            result, ret = self._mid_cost_extract(query, memories)
        else:  # HIGH
    ...
    def _build_memories_text(self, memories: List[Any]) -> str:
        """Build memories text with metadata"""
        memories_text = ""
        for idx, mem in enumerate(memories):
            mem_metadata = []
            if hasattr(mem, 'date_time') and mem.date_time:
    ...
    def _low_cost_extract(self, query: str, memories: List[Any]) -> Tuple[List[str], List]:
        """Low cost: Simple direct prompt"""
        if len(memories) == 0:
            return [], []

        memories_text = self._build_memories_text(memories)
        prompt = MODULE2_ENTITY_RELATION_PROMPT_Direct.format(query=query, memories_text=memories_text)

        task_args = [(0, prompt, self.args)]
        ret = get_llm_response(args=self.args, task_args=task_args, disable_internal_threading=True)
        return self._parse_relations(ret), ret

    def _mid_cost_extract(self, query: str, memories: List[Any]) -> Tuple[List[str], List]:
        """Mid cost: CoT prompt"""
        if len(memories) == 0:
            return [], []

        memories_text = self._build_memories_text(memories)
        prompt = MODULE2_ENTITY_RELATION_PROMPT_COT.format(query=query, memories_text=memories_text)

        task_args = [(0, prompt, self.args)]
        ret = get_llm_response(args=self.args, task_args=task_args, disable_internal_threading=True)
        return self._parse_relations(ret), ret

    def _high_cost_extract(self, query: str, memories: List[Any]) -> Tuple[List[str], List]:
        """High cost: Plan→Act→Reflect prompt"""
        if len(memories) == 0:
            return [], []
    ...
    def _parse_relations(self, ret) -> List[str]:
        ...
        try:
            ...
            if "<answer>" in response and "</answer>" in response:
                ...
                if end != -1:
                    response = response[start:end].strip()
                # print("[Module2_EntityRelation] response: ", response)
            ...
            try:
                ...
                try:
                    ...
                    print(f"[Module2_EntityRelation._parse_relations] JSON repair failed: {repair_error}, returning empty list")
                    return []
            ...
            print(f"[Module2_EntityRelation._parse_relations] Error: {e}")
...
class Module3_TemporalRelation:
    ...
    def _low_cost_extract(self, query: str, memories: List[Any]) -> Tuple[List[str], List]:
        ...
        memories_text = self._build_memories_text(memories)
        prompt = MODULE3_TEMPORAL_RELATION_PROMPT_Direct.format(query=query, memories_text=memories_text)
    ...
    def _mid_cost_extract(self, query: str, memories: List[Any]) -> Tuple[List[str], List]:
        ...
        memories_text = self._build_memories_text(memories)
        prompt = MODULE3_TEMPORAL_RELATION_PROMPT_COT.format(query=query, memories_text=memories_text)
    ...
    def _parse_relations(self, ret) -> List[str]:
        ...
        try:
            ...
            if "<answer>" in response and "</answer>" in response:
                ...
                if end != -1:
                    response = response[start:end].strip()
                # print("[Module3_TemporalRelation] response: ", response)
            ...
            try:
                ...
                try:
                    ...
                    print(f"[Module3_TemporalRelation._parse_relations] JSON repair failed: {repair_error}, returning empty list")
                    return []
            ...
            print(f"[Module3_TemporalRelation._parse_relations] Error: {e}")
...
class Module4_Summary:
    ...
    def execute(
        ...
        model_name = getattr(self.args, 'model', 'meta/llama-3.3-70b-instruct')
        if cost_level == CostLevel.LOW:
            result, ret = self._low_cost_summary(query, memories, entity_relations, temporal_relations, topic_relations)
        elif cost_level == CostLevel.MID:
            result, ret = self._mid_cost_summary(query, memories, entity_relations, temporal_relations, topic_relations)
        else:  # HIGH
    ...
    def _build_context(self, query: str, memories: List[Any], entity_relations: List[str], temporal_relations: List[str]) -> str:
...
"""
    ...
    def _low_cost_summary(self, query: str, memories: List[Any], entity_relations: List[str], temporal_relations: List[str], topic_relations: List[str] = None) -> Tuple[str, Any]:
        """Low cost: Simple direct prompt"""
        if len(memories) == 0:
            return "", []
        ...
        prompt = MODULE4_SUMMARY_PROMPT_Direct.format(query=query, entity_text=entity_text, temporal_text=temporal_text, topic_text=topic_text)

        task_args = [(0, prompt, self.args)]
        ret = get_llm_response(args=self.args, task_args=task_args, disable_internal_threading=True)
        return self._parse_summary(ret), ret

    def _mid_cost_summary(self, query: str, memories: List[Any], entity_relations: List[str], temporal_relations: List[str], topic_relations: List[str] = None) -> Tuple[str, Any]:
        """Mid cost: CoT prompt"""
        if len(memories) == 0:
            return "", []
        ...
        prompt = MODULE4_SUMMARY_PROMPT_COT.format(query=query, entity_text=entity_text, temporal_text=temporal_text, topic_text=topic_text)

        task_args = [(0, prompt, self.args)]
        ret = get_llm_response(args=self.args, task_args=task_args, disable_internal_threading=True)
        return self._parse_summary(ret), ret

    def _high_cost_summary(self, query: str, memories: List[Any], entity_relations: List[str], temporal_relations: List[str], topic_relations: List[str] = None) -> Tuple[str, Any]:
        """High cost: Plan→Act→Reflect prompt"""
        if len(memories) == 0:
            return "", []
    ...
    def _parse_summary(self, ret) -> str:
        ...
        if "<answer>" in response and "</answer>" in response:
            ...
            if end != -1:
                ...
                # print("[Module4_Summary] response: ", extracted)
                return extracted

        # Debug print the raw response we will attempt to parse
        # print("[Module4_Summary] response: ", response)
...
class Module5_TopicRelation:
    ...
    def execute(
        ...
        Returns:
            (Topic relation list, cost value (based on token count))
        """
        if cost_level == CostLevel.NOOP:
            return [], 0.0
    ...
    def _low_cost_extract(self, query: str, memories: List[Any]) -> Tuple[List[str], List]:
        ...
        memories_text = self._build_memories_text(memories)
        prompt = MODULE5_TOPIC_RELATION_PROMPT_Direct.format(query=query, memories_text=memories_text)
    ...
    def _mid_cost_extract(self, query: str, memories: List[Any]) -> Tuple[List[str], List]:
        ...
        memories_text = self._build_memories_text(memories)
        prompt = MODULE5_TOPIC_RELATION_PROMPT_COT.format(query=query, memories_text=memories_text)
    ...
    def _parse_relations(self, ret) -> List[str]:
        """Parse LLM response to extract topic relations"""
        if len(ret) == 0:
            return []
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    print(f"[Module5_TopicRelation._parse_relations] JSON repair failed: {repair_error}, returning empty list")
                    return []
            ...
            print(f"[Module5_TopicRelation._parse_relations] Error: {e}")
...
class ModularPipelineExecutor:
    ...
    def __init__(
        ...
        # Action counter (includes noop stats)
        self.action_counts = {
            'module1_low': 0, 'module1_mid': 0, 'module1_high': 0, 'module1_noop': 0,
            'module2_low': 0, 'module2_mid': 0, 'module2_high': 0, 'module2_noop': 0,
            'module3_low': 0, 'module3_mid': 0, 'module3_high': 0, 'module3_noop': 0,
            'module5_low': 0, 'module5_mid': 0, 'module5_high': 0, 'module5_noop': 0,
            'module4_low': 0, 'module4_mid': 0, 'module4_high': 0, 'module4_noop': 0,
        }

        # Total cost tracking
        self.total_cost = 0.0

        # Noop penalty tracking - set to a huge penalty to discourage noop
        self.noop_penalty = 0.01  # Huge penalty for each noop action
        self.noop_count = 0       # Track total noop actions taken

    def _aggregate_embeddings(self, memories: List[Any], dim: int = 768) -> torch.Tensor:
        """Aggregate embeddings of memories into a single embedding (average pooling)"""
        if len(memories) > 0:
    ...
    def _text_list_to_embedding(self, texts: List[str], dim: int = 768) -> torch.Tensor:
        ...
        # Combine text list into a single text
        combined_text = "\n".join(texts)
        ...
        # Return zero vector if no encoder or encoding fails
        return torch.zeros(dim).to(self.device)
    ...
    def execute(
        ...
        # ========== Step 1: Module1 decision (based on query + initial_memories) ==========
        ...
        total_cost += cost1
        # Add noop penalty if action was noop
        if m1_action == CostLevel.NOOP:
            total_cost += self.noop_penalty
            self.noop_count += 1

        # ========== Step 2&3&5: Module2, 3, and 5 decisions (based on query + filtered_memories) ==========
        ...
        total_cost += cost5
        # Add noop penalty if action was noop
        if m5_action == CostLevel.NOOP:
            total_cost += self.noop_penalty
            self.noop_count += 1

        # ========== Step 4: Module4 decision (based on query + aggregated_memory_emb) ==========
        # Calculate and aggregate embeddings based on actual outputs from Module2, 3, and 5
        ...
        # Aggregate entity_emb, temporal_emb, and topic_emb into a single memory_emb (average pooling, same dimension as Module1-5)
        # If all are empty (return zero vector), use filtered_memory_emb as fallback
        ...
        # ========== Step 5: Module4 FINAL (Integrates everything) ==========
    ...
    def _update_action_count(self, module: str, action: int):
        """Update action count"""
        level = ['low', 'mid', 'high', 'noop'][action]
        key = f'{module}_{level}'
        self.action_counts[key] += 1

    def get_noop_stats(self) -> Dict[str, Any]:
        """Get noop statistics"""
        return {
            'noop_count': self.noop_count,
            'noop_penalty_per_action': self.noop_penalty,
            'total_noop_penalty': self.noop_count * self.noop_penalty,
            'noop_action_counts': {
                k: v for k, v in self.action_counts.items() if k.endswith('_noop')
            }
        }

    def set_noop_penalty(self, penalty: float):
        """Set noop penalty value"""
        if penalty < 0:
            raise ValueError("Noop penalty must be non-negative")
        self.noop_penalty = penalty

    def reset_noop_stats(self):
        """Reset noop statistics"""
        self.noop_count = 0
        # Reset noop action counts
        for key in self.action_counts:
            if key.endswith('_noop'):
