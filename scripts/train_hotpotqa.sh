...
WANDB_PROJECT="hotpotqa-training"
...
COST_STRATEGY="prompt_tier"  # Options: rule_llm, prompt_tier, model_tier
REWARD_WEIGHT=1
COST_WEIGHT=0
tag='llm'

DATA_FILE="${DATA_FILE:-./data/hotpotqa/hotpotqa_7k.json}"
TEST_DATA_FILE="${TEST_DATA_FILE:-./data/hotpotqa/eval_200.json}"
...
WANDB_RUN_NAME="ppo_${MODEL_NAME##*/}_${RETRIEVER_NAME}_${COST_STRATEGY}_rw${REWARD_WEIGHT}_cw${COST_WEIGHT}_${tag}_${TIMESTAMP}"
...
mkdir -p ./logs_hotpotqa

# Generate log filename (includes timestamp and cost-strategy)
LOG_FILE="./logs_hotpotqa/training_${COST_STRATEGY}_${tag}_${TIMESTAMP}_${COST_WEIGHT}_${MODEL_NAME##*/}.log"
...
echo "DATA_FILE: $DATA_FILE" >> $LOG_FILE
echo "TEST_DATA_FILE: $TEST_DATA_FILE" >> $LOG_FILE
echo "===========================" >> $LOG_FILE
echo "" >> $LOG_FILE


# Use -u flag to disable output buffering, ensuring print statements output immediately
python -u train/train_hotpotqa.py --data-file ${DATA_FILE} --test-data-file ${TEST_DATA_FILE} --model ${MODEL_NAME} --api --retriever ${RETRIEVER_NAME} --llm-judge --cost-strategy ${COST_STRATEGY} --reward-weight ${REWARD_WEIGHT} --cost-weight ${COST_WEIGHT} 2>&1 | tee -a $LOG_FILE
